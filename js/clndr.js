...
(function (factory) {
  ...
  var clndrTemplate = "<div class='row month'><div class='container-fluid'>" +
    "<div class='clndr-previous-button col-xs-4 col-sm-4'>&lt;&lt;前の月</div><div class='col-xs-4 col-sm-4 text-center'><%= month %> <%= year %></div><div class='col-sm-4 text-right clndr-next-button'>次の月&gt;&gt;</div>" +
    "</div></div>" +
    ...
    "<% for(var i = 0; i < numberOfRows; i++){ %>" +
      ...
      "<% for(var j = 0; j < 7; j++){ %>" +
        "<% var d = j + i * 7; %>" +
        "<td class='<%= days[d].classes %> text-center'><div class='day-contents'><%= days[d].day %>" +
        "</div></td>" +
      "<% } %>" +
      "</tr>" +
      "</table>" +
